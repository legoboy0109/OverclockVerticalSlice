...
# types (see their doc comments) — real Combat (ADR-0010) does not exist yet,
# so the end-to-end test drives them via a throwaway registered verb whose
# apply() appends a StructureDestroyedEvent directly, mirroring
...
func test_hq_destroyed_through_apply_action_end_to_end_sets_game_over_and_next_action_rejected() -> void:
	...
	next_action.verb = Action.Verb.ATTACK
	...
	# Cleanup.
	GameState.unregister_verb(Action.Verb.ATTACK)
	assert_bool(GameState._validators.has(Action.Verb.ATTACK)).is_false()
...
func test_win_condition_mid_turn_sets_game_over_and_leftover_ap_cannot_be_spent() -> void:
	...
	followup.verb = Action.Verb.ATTACK
	followup.player = 0
