...
**GDD**: `design/gdd/base-production.md` — Core Rule 7 (producing a unit); Edge Cases "Production & deploy"; the "Production (Rule 7)" ACs (minus the start-of-turn reset, which is Story 003).
...
- **Start-of-turn reset of `units_produced_this_turn`** is **Story 003** — not here. This story assumes the counter is whatever state passes in.

---

## Out of Scope

- Start-of-turn reset of `units_produced_this_turn` (→ 0) — Story 003 (Rule 7 reset half).
...
- **Depends on**: Story 001 (schema/registry — `producible_types`, `BuildStatus.COMPLETED`, `StructureTypes`). Realistic fixtures reach a Completed producer via Stories 002 (build) + 003 (advance); pure tests may fabricate a Completed producer directly.
