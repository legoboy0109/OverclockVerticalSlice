...
| **2** | ✅ **DONE 2026-08-25 (S8-05)** — accent coverage is now gated | `tests/unit/art/accent_coverage_test.gd` — in the **suite**, not the pipeline | ★ Put in the suite deliberately: a pipeline check runs when someone remembers, and this defect got through precisely because nothing ran automatically. Four assertions: a coverage floor (30%) and ceiling (75%) per owned-faction sprite · **neutral must stay achromatic** (≤10%, art-bible §4.2 — this one runs the *opposite* way, and a first draft that demanded coverage everywhere would have failed correct art) · rush/boom agree within 3 points (they are recolours of one master) · roster spread ≤2.5× (it was **4.7×** with this defect live). Verified against `analyse_legibility.py` — the two implementations agree to ±0.1 points — and verified able to fail by raising the floor |
