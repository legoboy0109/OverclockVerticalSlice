...
| ASSET-012 | Builder | Infantry (4 facings) | rush/boom/neutral | Done (idle) | ★ **Squat four-legged CARRIER walker** — body plan chosen by user 2026-08-25; **re-proportioned shorter and bulkier at user request 2026-08-26** (`builder_rush_r6_c2.png`, seed 1510903717, superseding r3_c2 which read too tall and leggy). The body now dominates the silhouette and the legs are short and crouched. ⚠ The overall sprite aspect barely moved (0.95 → 0.97) — what changed is the **body-to-leg ratio**, which no bounding-box metric detects; judge it by eye. ⚠ Three rounds to get there: shrinking the legs makes SDXL **delete** them (legless sled, tracked barge), and piling on bulk words displaced the grey/four-leg clauses entirely. **Crouching the legs is the lever, not scaling them.** ★ **Cargo cradle DRAWN and composited 2026-08-26** (`tools/asset-pipeline/draw_cargo_cradle.py`) after two further generation rounds could not put a legible one on its back without undoing the body — prompt weight is finite, so strengthening the cradle clause restored the shadow/orange/long-leg failures. Sprite grew 145 → 157 px tall as a result; a visible cradle and a minimal silhouette are opposed and this is the chosen point on that trade. Its cast shadow WAS liftable (machine luma 99 vs shadow 117 — unlike r2, where they measured identical), via a border flood plus an enclosed-pocket clear for the patch walled in by the legs. The roster's only support unit; defenceless and consumed by the structure it raises, so the silhouette must read "not a soldier". Sized by **width** at 140px — narrower than the Scout's 148 so the two four-legged units stay tellable apart. ★★ **ACCENT COVERAGE RAISED 22.8% → 33.9% on 2026-08-26 (S8-19, user's call).** The shipped Builder carried faction hue on **22.8%** of its body against the S8-05 gate's **30% floor** — 7 points under, and a roster spread of 2.75× against a 2.5× ceiling. ⛔ **It never tripped the gate, because `accent_coverage_test.gd` kept a hand-written archetype list the Builder was not in** — the identical defect S8-14 had just fixed in the two sibling guards, left live in the one written earlier for the same purpose. ⇒ *Fixing an anti-pattern where you found it is not fixing it.* Fixed the **art**, not the threshold, exactly as the test's own failure message instructs: `promote_accent.py --target 34` grows the **existing** accent regions outward (dilation decides how far the accent extends, never where it belongs — brightest-N and value-banded approaches both measured correct and looked like speckle and corduroy). **Silhouette untouched.** ★ 34 was chosen **by eye** from rendered candidates at shipped scale, not as the smallest passing number: at 42% the Builder reads level with the Trooper (42.5%) and looks like a combat unit, which fights the brief; at 33.9% the grey upper hull still dominates, so *'not a soldier'* survives **and** it remains the roster's least-coloured unit by 8.6 points. Gate now enumerates `UnitTypes.ALL`; roster spread 1.85×. Evidence: `production/qa/evidence/s8-14-builder/`. ✅ **BASE LOOK APPROVED by the user 2026-08-26** — *"it looks good"* — the first explicit sign-off this entry has carried. ★★ **CRADLE RE-SEATED the same day (S8-20)** on the follow-up note *"the crate on the back looks a bit tacked on"*. S8-16 had already named this failure mode and its contact treatment was simply too weak. **Four measured causes**: (1) ⛔ **value** — the cradle's lit face was **150** against a hull median of **114**, because the palette was matched to the hull's LIT PANELS rather than its median, and a lighter object on a darker surface pops forward regardless of what else is right; (2) INK outlined the **whole** box including edges buried in the hull — a hard line all the way round is the strongest sticker cue, since real geometry has no outline where it meets another surface; (3) both near rim edges were one flat ACCENT, reading as a decal outline rather than an edge catching light; (4) the contact shadow was a **rectangle** over a **rhombus** footprint, so it darkened hull the cradle never touches. Fixed respectively by toning to 124/102, inking silhouette edges only, splitting the rim into ACCENT_LIT / ACCENT_SHADE (both kept inside recolor.py's gate so they still re-key), and distance-fielding the occlusion off the cradle's own alpha. ★★ Plus **mounting brackets** — occlusion says *resting*, hardware says *fixed*. ⚠ **Rejected on render**: a wider cargo bed (0.60–0.68) **overhangs the hull silhouette** and reads as a tray balanced on top; a taller box reads as MORE separate, because prominence and integration pull opposite ways. ✅ **The height trade went the right way**: seating it INTO the hull instead of on top means less canvas growth, **157 → 154 px**, recovering a little of what S8-16 cost S8-15's "shorter". ★ `builder_rush_r6_c2_precradle.png` is now kept in `art-source/cleaned/` so the cradle can be re-cut without git archaeology. Full post-mortem — including round 1's orange-dominance and round 2's genuinely unremovable shadow — in `generation-prompts.md` |
