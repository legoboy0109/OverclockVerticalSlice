...
| ASSET-012 | Builder | Infantry (4 facings) | rush/boom/neutral | Done (idle) | ★ **Squat four-legged CARRIER walker** — body plan chosen by user 2026-08-25; **re-proportioned shorter and bulkier at user request 2026-08-26** (`builder_rush_r6_c2.png`, seed 1510903717, superseding r3_c2 which read too tall and leggy). ⚠ The overall sprite aspect barely moved (0.95 → 0.97) — what changed is the **body-to-leg ratio**, which no bounding-box metric detects; judge it by eye. ⚠ Three rounds to get there: shrinking the legs makes SDXL **delete** them (legless sled, tracked barge), and piling on bulk words displaced the grey/four-leg clauses entirely. **Crouching the legs is the lever, not scaling them.** ✅ **BASE LOOK APPROVED by the user 2026-08-26** — *"it looks good"*. ★★ **CARGO CRADLE ABANDONED 2026-08-26 (S8-22) after FOUR rounds — the Builder ships with none, user's call.** Two generation rounds failed structurally (prompt weight is finite: describing the cradle strongly enough to appear undid the body fixes of rounds 1–6). A drawn-and-composited one shipped (S8-16) and drew *"looks a bit tacked on"*; S8-20 fixed four genuine defects — value 150 vs a hull median of 114, an outline traced all the way round, a flat decal-like rim, and a rectangular shadow under a rhombus footprint — and the symptom did not move. ⛔ **S8-21 found the real cause: the hull is NOT in the projection it appears to be.** Measured off its own silhouette, the top-left edge runs at **−0.179** against a true dimetric **−0.500** (64% off) and it is **asymmetric**; every drawn cradle was a perfect symmetric 2:1 rhombus, so the crate sat on the machine **in a different projection**. Matching the measured slopes helped and still was not right. ★ **The silhouette was always doing the work** — the hunched, back-heavy body reads as a carrier and not a soldier with no cargo box at all. ✅ Removal **reversed every cost**: sprite **157 → 145 px**, fully restoring S8-15's "shorter"; `draw_cargo_cradle.py` deleted; accent re-derived to **34.7%**, clearing the S8-05 floor. ⚠ **If a cradle is ever wanted again, re-roll the body with the bed IN the render — never composite one.** Full post-mortem in `generation-prompts.md`. Sized by **width** at 140px — narrower than the Scout's 148 so the two four-legged units stay tellable apart. The roster's only support unit; defenceless and consumed by the structure it raises. Evidence: `production/qa/evidence/s8-14-builder/` |
