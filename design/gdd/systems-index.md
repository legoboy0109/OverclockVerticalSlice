...
> **Status**: Approved
> **Created**: 2026-07-19
> **Last Updated**: 2026-08-05 (economy pivot — name + two-pool descriptions)
...
**two-budget tempo economy** *(revised 2026-08-05 from a single-AP pool — see ap-economy.md)*:
a flat per-turn **AP** budget for moving/fighting and a banked **Credits** war chest for
build/produce/research, where every economic action still spends a small AP surcharge so
investment stays tempo-linked. Victory goes to whoever compounds tempo fastest. The system set
is therefore small and tightly coupled: a foundational board + turn + economy layer, a core
layer of gameplay systems that spend from the two budgets, a presentation layer built around a
readable pre-commit action interface (Pillar 3), and two Feature systems (AI, factions).
...
| 3 | AP & Credits Economy | Economy | Vertical Slice | ★★ **In Revision — ECONOMY RE-BASED ONTO RESEARCH 2026-08-24** (user): Economy Outposts deleted; `credit_income` = `BASE_INCOME` (**1,000**) + three finite research tiers × **+500** = **hard ceiling 2,500**. ★ **Also rescaled 2026-08-24:** all Credit quantities ×100 and `FLAT_AP_PER_TURN` 10 → **30** (AP action costs unchanged), for finer tuning granularity and to let larger armies actually act. All outpost-count terms removed. ★ This is a PIVOT fix — a finite tech tree is a hard ceiling where the old tiering curve was only a soft brake. **Prior: In Revision — PIVOT 2026-08-05** (single AP pool split into AP (flat tactical, +carryover) + Credits (banked economy); see ap-economy.md; re-review + `/propagate-design-change` pending). Prior: Approved (2026-07-22: the dominant-strategy blocker that flagged this — the Economy-Tech + boom + large-map composition — is **resolved by pinning the VS board to a fixed 14×16** (grid-terrain.md); the map-size "Bimodal meta" Open Question is closed and the last owed Turn-Manager start-of-turn cite added. Building-count cap `MAX_OUTPOST_COUNT` deliberately left disabled pending sim/playtest, per user. Full `/review-all-gdds` re-run queued to confirm corpus) | design/gdd/ap-economy.md | Game State & Turn Manager |
| 4 | Unit System | Gameplay | Vertical Slice | Approved (re-flagged Needs Revision by `/review-all-gdds` 2026-07-22 for 3 stale "Movement In Revision" / stale own-status cross-references — fixed same session; reverted to Approved) | design/gdd/unit-system.md | Grid & Terrain, Game State, AP & Credits Economy, Research (buff flags) |
| 5 | Movement System | Gameplay | Vertical Slice | Approved (re-reviewed 2026-07-21; formula confirmed sound, 3 blocking AC/precondition gaps fixed in-file; numbers spike-gated. **Additive contract 2026-07-22 via #9 propagation:** `reachable()` return now `{tile, min_cost, is_surcharged}` — purely additive, verdict unchanged) | design/gdd/movement-system.md | Grid & Terrain, Unit System, AP & Credits Economy, Game State |
| 6 | Combat Resolution | Gameplay | Vertical Slice | Approved (re-flagged Needs Revision by `/review-all-gdds` 2026-07-22 for sharing the undefined Research-Lab-destruction trigger path — fixed same session with a cross-doc note pointing to research-tech.md's new trigger-mechanism clarification; reverted to Approved. **Additive contract 2026-07-22 via #9 propagation:** new `legal_targets(unit, from_tile)` hypothetical-tile overload — purely additive, verdict unchanged) | design/gdd/combat-resolution.md | Unit System, Grid & Terrain, AP & Credits Economy |
| 7 | Base & Production | Gameplay | Vertical Slice | ★★ **In Revision — STRUCTURE ROSTER REBUILT 2026-08-24** (user): Economy Outpost **deleted**; Production Outpost → **Barracks** (produces infantry + grants infantry cap); new **Factory** (ground vehicles) and **Airfield** (air); **per-faction maximum on every structure**. ★ The maximums are the PIVOT fix — a player runs out of things to build, so AP finally reaches movement. **Prior: Approved** (re-flagged Needs Revision by `/review-all-gdds` 2026-07-22 for a non-reciprocal dependency vs. research-tech.md — fixed same session: added Research/Tech as a Hard downstream dependent + reworded stale Provisional tag; reverted to Approved) | design/gdd/base-production.md | Grid & Terrain, AP & Credits Economy, Unit System |
| 8 | Research / Tech | Progression | Vertical Slice | ★★ **In Revision — RESEARCH IS NOW THE ECONOMY 2026-08-24** (user): Economy Tech becomes a strictly sequential three-tier line (10/20/35 Credits, +5 income each). The old outpost-scaled `economy_tech_income_bonus` and its compounding sanity check are **obsolete**. ★ The Research Lab is now the single most valuable structure in the game and a natural attack target. **Prior: In Revision — PIVOT 2026-08-05** (AP↔Credits split, see ap-economy.md / #3): `research_cost` now denominated in **Credits** (values unchanged) + a new per-tech **`ap_surcharge`** (AP, base 1) — research is a dual-cost both-or-neither action; Economy Tech's income bonus now feeds **`credit_income`**; Research Lab `build_cost` is Credits + `BUILD_AP_COST`. `/design-review` on the delta pending. Prior: Approved (2026-07-22: the shared Economy-Tech + boom + large-map dominant-strategy composition **resolved via the 14×16 board pin** — see #3; the "which tech" Economy-Tech-vs-Attack/Defense concern remains a non-blocking playtest-routed Open Question) | design/gdd/research-tech.md | AP & Credits Economy, Unit System, Base & Production |
| 9 | Command & Action Interface | UI | Vertical Slice | Approved | design/gdd/command-action-interface.md | Movement, Combat, Base & Production, AP & Credits Economy |
| 10 | Game HUD (inferred) | UI | Vertical Slice | **Needs Revision** (2026-08-24 cross-review B-3/W-2: income breakdown rewritten to `gross − upkeep = net`; pip-vs-numeric threshold invalidated by vehicle hp up to 28 and flagged to `/ux-design`). Prior: Approved (re-review 2026-07-22, full 8-specialist + creative-director: the 5 first-pass blockers confirmed closed; 2 new blockers found — CR-3a whose-AP/turn-boundary/GameOver rules + audio total priority order — both **fixed in-file same session** along with 9 recommended items (OQ-5 closed via Redot binary verification, OQ-8 downgraded, AC-1 split, CR-11 coverage, etc.). CD verdict: Approved once the 2 blockers close, no third spin needed) | design/gdd/game-hud.md | Game State, AP & Credits Economy, all Core |
| 11 | AI Opponent | Gameplay | Vertical Slice | **Needs Revision** (2026-08-24 cross-review B-1/B-5: `economy_value` re-pointed at research tiers; ★ `CREDIT_TO_AP_RATE` 1.0 → 0.01 after the ×100 rescale broke its 1:1 anchor — at 1.0 the AI would have valued building and killing ~100× a march). Prior: Approved (confirmation re-review 2026-07-22, full 6-specialist + creative-director: prior pass's 5 blocking clusters confirmed held with zero constant drift; 3 new blockers found — LETHAL_FLOOR_BONUS cross-knob invariant, AC-20/"forward entity" contradiction, standing-start passivity — all **fixed in-file same session** (positional/retreat scoring re-normalized by tiles-traversed; nearest-enemy simplification; economy-ceiling invariant guard) + ~12 recommended items folded in. CD verdict: APPROVED once the 3 close, no further pass needed. No registry/shared-fact changes) | design/gdd/ai-opponent.md | Game State (read interface), all Core |
| 12 | Faction Identity | Gameplay | Vertical Slice | ★★ **IN REVISION — framework v2, 2026-08-24** (six independently-rostered factions; CR-6 combat-stat lock UNLOCKED via OQ-9's named reopen-trigger; CR-2 6 domains → 9, reshaped into OWN vs MOD; CR-3 narrowed to "no **unpriced** verb"; new CR-8/9/10. Needs a fresh `/design-review`. Spawns systems #15–21. **Prior v1 status:** Approved (2026-07-22 confirmation re-review, 2nd full 6-specialist + creative-director pass: prior deciding blocker — the `ap_income` formula omitting AP & Credits Economy's Economy-Tech term + `ECONOMY_TECH_TIER_THRESHOLD` cap — **confirmed fully resolved** [corrected to the full 4-term form; verified byte-for-byte vs ap-economy.md by economy-designer + systems-designer independently]. 2 new blockers found + fixed in-file same session: (a) `production_cap` cap-0 boundary gap — a faction delta on a base-cap-0 non-producer (Research Lab / Defensive Structure) was undefined and would have forced cap 0→1; now inert, so a faction delta crosses the produces/doesn't-produce boundary in **neither** direction [Formulas 4c, CR-2.4, Edge Cases, AC-12]; (b) FSM had no pre-lock preview sub-state for the AC-27 acknowledgment + AC-28 preview contracts — added a **SELECTING** sub-state [States, AC-26]. Plus 2 cleanups: AC-4a "executable today" softened; Overview provisional-framework marker + OQ-9 reopen-trigger. User accepted the fixes and marked Approved without a 3rd pass. **Framework-only — all Rush/Boom values remain prototype-gated. The 10th and final VS system Approved.** See design/gdd/reviews/faction-identity-review-log.md. Additive `effective_X` contracts owed to 5 upstream GDDs via `/propagate-design-change`, deferrable — no-op under the Neutral default the VS ships) | design/gdd/faction-identity.md | AP & Credits Economy, Unit System, Base & Production, Combat |
| 13 | Persistence & Campaign | Persistence | Alpha (deferred) | Not Started | — | Game State, all Core |
| 14 | ~~Vehicle & Mech Tier~~ | Gameplay | ~~Full Vision (deferred)~~ | ★ **SUPERSEDED 2026-08-24 by #17 Unit Classes** — the faction corpus v2 pulls vehicles and air forward from Full Vision and generalises them into a three-class taxonomy. Retained as a row so a reader of the old index finds its disposition | design/gdd/unit-classes.md | — |
| 15 | **Unit Upkeep** | Economy | ★ **Sprint 6** | **Draft** (2026-08-24) — the Credit drain. ★ **This is the PIVOT fix**: the vertical slice failed because Credits were unbounded (peak 5,724, still climbing). Turns gross income into `net_credit_income`. The one Tier-1 doc that gets built rather than only designed | design/gdd/unit-upkeep.md | AP & Credits Economy, Game State, Base & Production, Unit System |
| 16 | **Population Cap** | Gameplay | Faction wave 1 | **Draft** (2026-08-24) — the infantry cap, its escalating raise curve, and cap exemption. ★ Paired with #15: cap = how many you may field, upkeep = how long you can afford them | design/gdd/population-cap.md | Base & Production, Unit System, Unit Upkeep |
| 17 | **Unit Classes** | Gameplay | Faction wave 2 | **Draft** (2026-08-24) — infantry / ground vehicle / air, and what class membership means for terrain, cover, targeting and occupancy. ★ Largest single addition in the corpus; supersedes #14 | design/gdd/unit-classes.md | Grid & Terrain, Movement, Combat, Unit System |
| 18 | **Damage Types** | Gameplay | Faction wave 3 | **Draft** (2026-08-24) — `KINETIC`/`EMF`/`INCENDIARY` + area shapes. ★ Extends Combat's one formula by exactly one additive term; `KINETIC` is neutral so no shipped matchup changes. **Base-game symmetric content, not a faction lever** | design/gdd/damage-types.md | Combat Resolution, Unit System |
| 19 | **Unit Abilities** | Gameplay | Faction wave 2 | **Draft** (2026-08-24) — the shared, base-game-priced ability catalogue. ★★ **This is what makes CR-3 enforceable**: factions select from the catalogue (D8) and may never add to it or re-price it | design/gdd/unit-abilities.md | Command & Action Interface, AP & Credits Economy, Combat, Unit Upkeep |
| 20 | **Transport & Pilots** | Gameplay | Faction wave 2 | **Draft** (2026-08-24) — carriage and crewing; the **unpiloted** vehicle state. ★ The Independents' pirate is meaningless without it; the Machinist's Union is built on it | design/gdd/transport-and-pilots.md | Unit Abilities, Unit Classes, Population Cap, Combat, Game State |
| 21 | **Promotion & Veterancy** | Progression | Faction wave 4 | **Draft** (2026-08-24) — field promotion, three hard-capped ranks, and rank loss through tech-base disruption. ★ The Holy Cosmic Empire cannot exist without it; every other faction can ship without it | design/gdd/promotion-veterancy.md | Combat Resolution, Unit System, Research, Base & Production |
...
> **Concept mapping**: The concept's shorthand "six faction-agnostic core systems" = AP & Credits Economy (#3),
...
| **Gameplay** | The systems that make the game fun | Unit, Movement, Combat, Base & Production, AI Opponent, Faction Identity, **Population Cap, Unit Classes, Damage Types, Unit Abilities, Transport & Pilots** |
| **Economy** | Resource creation/consumption | AP & Credits Economy, **Unit Upkeep** |
| **Progression** | How the player grows | Research / Tech, **Promotion & Veterancy** |
...
3. **AP & Credits Economy** — depends on: Game State. Two resources: **AP**, a flat 10/turn tactical budget whose unspent balance carries over capped at 5 (not lost), spent on move/attack plus a small AP surcharge on economic actions; and **Credits**, a banked economic pool with no cap (the old per-outpost income curve now funds it), spent as the main cost of produce/build/research. The economy underpinning Pillar 2 (Tempo Is the Skill) and the reworded Pillar 1 ("Every Choice Is a Tradeoff").

### Core Layer (depends on Foundation)

1. **Unit System** — depends on: Grid, Game State, AP & Credits Economy. Data-driven, statically-typed unit definitions and per-unit state (hp, atk, move cost, produce cost, position, hasAttacked). *(Owns the AP cost values Movement/Base & Production spend — matches the systems table above and the Unit GDD's Hard dependency on AP & Credits Economy.)*
...
5. **Research / Tech** — depends on: AP & Credits Economy, Unit, Base & Production. A **Research Lab** (5th structure, built via Base & Production for Credits + `BUILD_AP_COST`) unlocks 3 flat permanent techs, each researched for a **Credit** `research_cost` + a small per-tech **`ap_surcharge`** (dual-cost, both-or-neither) — Attack (+1 atk), Defense (+1 def), Economy (**research-keyed income tiers** — `base_income + econ_tier_bonus × tier`, `tier` capped at `max_economy_tier`; ★ S6-01 replaced the old per-Economy-Outpost bonus, and the Economy Outpost itself, with this); a lever for tempo swings.
...
| 3 | AP & Credits Economy | Vertical Slice | Foundation | systems-designer, economy-designer | M |
...
- None found. The AP & Credits Economy is a pure Foundation system queried by all spenders (Movement, Combat,
...
| AP & Credits Economy | Design / Balance | The two-budget (AP + Credits) balance center of the whole game | Largely de-risked by the concept prototype (no dominant opening); carry the prototype's baseline tuning values (REPORT.md) into the Formulas/Tuning sections |
