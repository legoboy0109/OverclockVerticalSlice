...
> **Implements Pillar**: Pillar 1 (One Economy, Every Choice — every structure and unit is paid from the single AP pool); Pillar 2 (Tempo Is the Skill — the "boom" half of the tempo duel; owns the endgame closeout-drag answer)
...
producer that can build only one basic unit type), the **Economy Outpost** (boosts AP income), the
...
and every unit produced, by spending from the single shared AP pool (Pillar 1) — there is no separate
"base-building phase" or parallel currency. The player engages with this system directly and
...
keeps building expensive in both AP and build time, and makes the Production Outpost a real
investment, so a losing player cannot cheaply spin up a cornered, unlimited unit factory to stall a
decided game.
...
**turning a corner of the map into a war engine**: you spend AP now — on an Economy Outpost that won't
earn back its cost for a few turns, on a Production Outpost that unlocks your heavy hitters — and you
...
> Economy** — this document owns the upfront cost and the vulnerable build window, not the payback
> formula (see Formulas, `economy_outpost_payback`, and `ap-economy.md`).
...
1. **A structure type is an immutable data template** with: display name, `hp` (max), `build_cost`
   (AP), `build_time` (owner-turns under construction), `production_cap` (units this structure may
...
2. **The four Vertical-Slice structures** (a fifth, the **Research Lab**, is **owned by Research / Tech
   ...
   - **Economy Outpost** — built during play. The **only** structure counted by
     `completed_outpost_count` (feeds `ap_income`'s tiered bonus). Does not produce units, does not
     attack. `build_cost` is a flat **4 AP** — **no research discount**. *(Research's **Economy Tech** was
     retuned 2026-07-21 from a build-cost discount to an `ap_income` bonus per completed Economy Outpost;
     it now flows through **AP Economy's income**, not this structure's cost. This supersedes the earlier
     `economy_outpost_discount` hook — see the Research/Tech #8 design-review; B&P re-review of this removal
     is owed, but no B&P number changes.)*
   ...
   **2b. The Research Lab (5th structure, Research-owned).** Research / Tech (#8) adds a **Research Lab**
   ...
   stats appear in the Section D table for roster completeness (hp 12 / `build_cost` 8 / `build_time` 2),
   tagged as Research-owned. It does not produce units (`production_cap = 0`) and does not feed
...
4. **Building a structure** spends `build_cost` AP via `spend()`, then places the structure on the
   chosen tile in the **Under-Construction** state. It occupies the tile immediately (blocking and
   targetable at once) but **produces no income, cannot produce units, and cannot attack until
   Completed**. It completes after `build_time` of the owner's turns elapse (Rule 6).
...
6. **Build completion is a start-of-turn effect, processed *before* the income snapshot.** At the
   owner's start-of-turn, each of their Under-Construction structures decrements its remaining build
   turns; any that reach 0 transition to **Completed** *first*, and only then is `ap_income` computed —
   so a just-completed Economy Outpost counts toward income that same turn. **The ordering is owned by
   Game State & Turn Manager's canonical start-of-turn sequence** (`game-state-turn-manager.md` Core
   Rule 3: step 3 *apply start-of-turn effects — incl. this build-timer advance* precedes step 4 *reset
   AP to income*). This GDD does not define the order — it contributes the build-timer-advance effect at
   step 3 and relies on the Turn Manager sequence to place it ahead of the income snapshot; AP Economy
   cites the same sequence for its snapshot timing.

7. **Producing a unit.** A **Completed** producer spends the unit's `produce_cost` AP (Unit-owned
   value) and creates the unit **instantly** (Unit System: units have no build time) on a
...
10. **Voluntary cancel.** On their turn, the owner may **cancel** their own Under-Construction
    structure, refunding `CANCEL_REFUND_RATE` (default 50%) of its `build_cost`; the structure is
    removed and its tile empties. **Completed** structures cannot be cancelled (only destroyed in
    combat). **Destruction in combat refunds nothing** (the boom punish, per AP Economy Rule 6).

11. **`completed_outpost_count(player)` — the AP Economy contract.** Returns the count of the player's
    **alive, owned, Completed Economy Outposts** — HQ excluded, Production Outposts and Defensive
    Structures excluded, Under-Construction excluded. This is the exact query `ap_income` consumes;
    sampled at the owner's start-of-turn snapshot (after Rule 6 completions).

12. **Closeout-drag brake (required mechanic — no parallel resource, no board clutter).** A losing
    player cannot cheaply stall a decided game, because: (a) a cornered **HQ produces only Scouts**
    (fragile hp-3 bodies that don't hold a line) at `production_cap = 2`/turn; (b) real production
    requires a **Production Outpost** — expensive in `build_cost` + `build_time`, **destroyable while
    building with no refund**, and itself capped at 4/turn; (c) all building competes for the **same
    AP** as defending. No hard structure cap is used in the VS (it remains a disabled tuning lever,
    Section G). Game State's optional `MAX_ROUNDS` + tiebreak is the backstop safety valve. *(Detailed
    drag scenarios in Edge Cases.)*
...
| AP Economy | `can_afford`/`spend` for `build_cost`, `produce_cost`, `DEFENSIVE_ATTACK_COST`; provides `completed_outpost_count` | `completed_outpost_count(player)` → `ap_income` | Base & Production owns the count query + all these costs' *timing*; AP Economy owns the pool and the income *amounts* |
...
| Structure | `hp` | `build_cost` (AP) | `build_time` (owner-turns) | `production_cap` (units/turn) | `producible_types` | `attack` | `attack_range` | `defense` | `can_counterattack` |
|-----------|------|-------------------|----------------------------|-------------------------------|--------------------|----------|----------------|-----------|---------------------|
...
|----------|-------|------|-------------|
| `DEFENSIVE_ATTACK_COST` | 1 | AP/attack | AP to fire a Defensive Structure — deliberately **< unit `attack_cost` (2)**, its reward for immobility |
| `CANCEL_REFUND_RATE` | 0.5 | fraction | Share of `build_cost` refunded on voluntary cancel of an under-construction structure |
| `MAX_OUTPOST_COUNT` | 10 (**disabled** in VS) | count | Hard ceiling on total outposts if ever enabled — off by default; documented so AP income has a nameable cap lever. **Not a pure no-op:** with the hard cap off, the only bound on `completed_outpost_count` is board-tile availability + AP. The registry's `ap_income` output range (`[10, 32]`) spans the un-teched *practical board-tile ceiling* (~26 at ≈12 outposts) up through the **Economy-Tech-boosted** ceiling (~32 — Research/Tech's per-completed-outpost income bonus, added 2026-07-21, capped by AP Economy's `ECONOMY_TECH_TIER_THRESHOLD` (6) per AP Economy's 2026-07-22 re-review — was ~38 uncapped); it reflects board-tile availability + AP (and now that research modifier, itself now capped), **not** this disabled lever — so a very large (24×24) board can still push `n` past that soft ceiling on the un-teched term. That residual runaway is the same one AP Economy flags as an open question; `MAX_OUTPOST_COUNT` is the lever that would re-impose a hard bound if playtest needs it. |

### `cancel_refund(structure)` — AP returned on voluntary cancel

`cancel_refund = floor(structure.build_cost × CANCEL_REFUND_RATE)`

| Variable | Symbol | Type | Range | Description |
|----------|--------|------|-------|-------------|
| `build_cost` | — | int | 4–9 (VS) | The structure's AP build cost |
| `CANCEL_REFUND_RATE` | — | float const | 0.5 | Refund fraction |
| `cancel_refund` | — | int | 2–4 (VS) | AP credited back to the pool on cancel |

**Output range:** `floor` keeps AP integer and rounds toward the harsher (owner-unfriendly) side,
matching `ap_income`'s conservative-clamp convention. **Only applies to voluntary cancel** — combat
destruction refunds nothing (0).
...
Total elapsed break-even from the turn AP is committed =
`build_time + ceil(build_cost / OUTPOST_BONUS_TIER1)` = `1 + 2 = 3` turns. `OUTPOST_BONUS_TIER1` (=2)
is **owned by AP Economy** — this GDD only sets `build_cost`/`build_time`. For the 5th+ outpost (tier 2,
+1/turn) payback stretches to `4/1 = 4` turns from completion — the intended diminishing-returns wall
on over-booming. **Worked example:** an Economy Outpost committed turn 3 (spend 4 AP) completes turn 4,
first bonus at turn 5's reset (+2), cumulative +2 / +4 / +6 by turns 5 / 6 / 7 — so cumulative bonus
equals the 4 AP spent at turn 6 (**break-even at turn 6**) and is strictly ahead from **turn 7 on**.

> **Sequencing note (added 2026-07-22 re-review):** `build_cost` is **sequence-invariant** — researching
> Economy Tech before or after building an outpost costs the identical flat 4 AP either way, since the
> tech no longer touches this cost (the old `economy_outpost_discount` hook is removed). But **income
> timing is not invariant**: Economy Tech's benefit is now a per-turn `ap_income` bonus (AP Economy,
...
The HQ costs a Sniper 10 hits × 2 AP = **≥20 AP in attacks alone** to destroy — and **materially more in
practice**: a Sniper first needs a Production Outpost (9 AP + 2 turns) and its own `produce_cost` (5 AP)
before firing, and with **no AP banking** (`ap_reset_policy`) those 20 attack-AP must be spread across
many turns at `BASE_INCOME` 10/turn while also paying to march the Snipers into range. A turn-3 HQ rush
is therefore impossible, yet a sustained late-game siege still finishes it (attrition-decided, never
unkillable). Nothing one-shots (min 2 hits). The Production Outpost dies to 3
Heavy hits (6 AP) — cheaper than its 9 AP build, keeping "defend it or lose your roster" real. Vs a
...
- **If the chosen build tile becomes occupied or otherwise illegal between selection and commit** (a
  ...
  and is **rejected** — no AP spent, no structure placed.
...
- **If the player cannot afford `build_cost`**: the build is not offered and `build()` rejects it — no
  AP spent (AP Economy `can_afford` gate + `apply_action` atomicity).
...
- **If an Economy Outpost completes at the owner's start-of-turn**: it transitions to Completed
  **before** the income snapshot that same turn (Rule 6), so it counts toward `ap_income` immediately —
  income reflects it that turn, not a turn late.
- **If several structures complete on the same start-of-turn**: all advance and complete before the
  single income snapshot; a batch of Economy Outposts finishing together all count that turn.
- **If an under-construction structure is destroyed in combat**: it is removed that resolution step, its
  tile empties, and **no AP is refunded** — the whole `build_cost` is lost (the boom punish, AP Economy
  Rule 6).
...
- **If a producer has already produced `production_cap` units this turn** (HQ 2 / Production Outpost 4):
  further production from it is **rejected** this turn, even with AP to spare — `units_produced_this_turn`
  gates it. It resets next start-of-turn.
...
- **If the producer itself no longer exists or is no longer Completed at commit**: `produce()`
  ...
  Completed status between preview and commit, **rejects**: no unit, no AP spent. *(Currently unreachable
...
- **If the owner cancels an under-construction structure**: it is removed, its tile empties, and
  `floor(build_cost × CANCEL_REFUND_RATE)` AP is credited back (Economy Outpost 2, Production Outpost 4,
...
- **If a losing player's Production Outpost is destroyed**: they fall back to **HQ Scout-only
  ...
  they must rebuild a Production Outpost: **9 AP + 2 build-turns exposed**, during which the winning
  player (who has board control) gets multiple free turns to finish the match or re-destroy the rebuild
  for as little as 6 AP. **The exchange is even more lopsided than 6-vs-9 raw AP suggests, because of the
  income differential:** a winning player who is up on the board is almost always up on completed Economy
  Outposts too, so their 6 AP re-destroy is a *marginal* spend from a larger income (e.g. 18–22 AP/turn),
  while the loser's 9 AP rebuild is the *majority* of a smaller income (often ≈`BASE_INCOME` 10 once their
  outposts are gone), leaving them nothing for defense that turn. The winner therefore affords both the
  re-destroy *and* continued pressure while the loser can afford only the rebuild — so the loser cannot
  out-rebuild the destruction, and a decided game closes out rather than dragging. *(Caveat — spike-gated:
  ...
  AP Economy) — the two were never jointly re-checked. Validate the re-destroy-vs-rebuild exchange holds
...
| AP Economy | Hard | `can_afford`/`spend` for `build_cost`, `produce_cost`, `DEFENSIVE_ATTACK_COST`; the cancel refund credits the pool |
...
|--------|--------------------------------------|
| AP Economy | `completed_outpost_count(player)` — the completed-Economy-Outpost count that feeds `ap_income` *(reciprocal: AP Economy is also upstream for `spend`)* |
...
*(Bidirectional note: Grid, Unit System, AP Economy, Game State, and Combat each already list Base &
Production as a Hard dependent in their own GDDs — the graph is reciprocal.)*
...
- **Research / Tech (#8)** — Base & Production has **zero live numeric ties reading from Research**
  ...
  removed — Economy Tech's benefit now flows entirely through AP Economy's `ap_income`, not through
  anything Base & Production owns). The only remaining link is hypothetical: **should structure `attack`
...
2. **→ AP Economy (#3, Approved):** this GDD resolves AP Economy's open "`completed_outpost_count`
...
4. **→ Game State & Turn Manager (#2, Designed) — LANDED 2026-07-21:** the canonical start-of-turn
   ordered sequence (flags → start-of-turn effects/build-timer advance → AP income snapshot) was added to
   `game-state-turn-manager.md` Core Rule 3 so build completions precede the income snapshot. Both this
   GDD (Rule 6) and AP Economy now reference that single sequence rather than each assuming the other
   enforces order.
5. **→ AP Economy (#3, Approved) — OWED:** AP Economy's start-of-turn income-snapshot rule should add a
   one-line cross-reference to the Turn Manager canonical sequence (Core Rule 3, step 4) so its snapshot
   timing cites the shared order. Additive doc note only — no formula/value change. Apply on AP Economy's
   next revision (or via `/propagate-design-change`).
...
| `PROD_OUTPOST_PRODUCTION_CAP` | 2–6 | 4 | Units/turn from a Production Outpost | High → mass-production swamps the one-pool tension | Low → forces multiple outposts |
...
| Parallel construction | allowed (AP-gated) | No per-turn build-order cap in the VS; flagged as an Open Question (could a one-turn AP windfall spawn many builds?). The no-banking rule largely closes this. |

> Structure stat values are **data-driven** (external `.tres`/config), tunable without code. Costs and
> `production_cap` interact with AP Economy's income tiers and the closeout-drag brake — retune the two
...
> 14×16; the map-size bimodality AP Economy flagged).
...
- **Build menu:** the buildable structures with their `build_cost` and `build_time`, affordability-gated
  (`can_afford` → available vs. greyed), per AP Economy's affordability rule.
...
- **Cancel affordance:** cancelling an Under-Construction structure must show the refund
  (`floor(build_cost×0.5)`) **before** confirming; mirrors Movement/Combat's cancel affordance.
...
> suite (deterministic, injected Grid + AP fixtures, no file I/O) plus Integration ACs that need real
> dependencies. `MAX_OUTPOST_COUNT` ACs are **infrastructure-only** (the cap is disabled in the VS — no
...
*Building (Rule 4):*
- **GIVEN** ≥ `build_cost` AP and a legal tile, **WHEN** `build(Economy Outpost, tile)`, **THEN** AP −4,
  structure placed Under-Construction, blocks/targets immediately.
...
*Production (Rule 7):*
- **GIVEN** a Completed Production Outpost (`units_produced_this_turn` 0) with ≥ `produce_cost` AP,
  **WHEN** `produce(Trooper, empty adjacent tile)`, **THEN** AP spent, Trooper created on the tile,
  counter →1.
- **GIVEN** the HQ, **WHEN** `produce(Heavy, …)`, **THEN** rejected (not in `producible_types`).
- **GIVEN** a producer at `production_cap`, **WHEN** another `produce()` with AP to spare, **THEN**
  rejected (cap gates independently of AP).
- **GIVEN** no empty adjacent tile, **THEN** production blocked, no AP spent.
...
- **GIVEN** a producer destroyed or no longer Completed between preview and commit, **WHEN** `produce()`
  commits, **THEN** rejected — no unit, no AP spent (mirrors the deploy-tile commit re-validation).
...
- **GIVEN** an Under-Construction Economy Outpost / Production Outpost / Defensive Structure, **WHEN**
  cancelled, **THEN** `floor(cost×0.5)` = **2 / 4 / 3** AP credited, structure removed, tile empties.
...
- **GIVEN** a fixture `S` and two independent clones (`A = clone(S)`, `B = clone(S)`), **WHEN** the same
  ...
  `has_attacked`, both players' AP pools, and the Grid occupancy map all match (no RNG, stable iteration
  order). *(State-equality is this field-wise comparison, not byte-level serialization — no hashing or
...
- **GIVEN** the VS config, **THEN** `MAX_OUTPOST_COUNT` reads **disabled** — an 11th Economy Outpost is
  **not** rejected on count grounds (the only build gate is tile availability + AP). *This one assertion
  IS worth a smoke check* (it guards the "disabled" claim); no further count-behavior coverage is
  required while the lever is off.

*Design-rule toggles (Section G — Config/Data smoke checks, guard against regression):*
- **GIVEN** sufficient AP for two builds in one turn, **THEN** both `build()` calls succeed the same turn
  (parallel construction is allowed, AP-gated — no per-turn build-order cap in the VS).
...
- **GIVEN** a player who has researched Economy Tech, **WHEN** they build an Economy Outpost, **THEN**
  `build_cost` is unmodified (flat **4** AP) — Economy Tech affects `ap_income` only, not `build_cost`;
  the removed `economy_outpost_discount` hook must not be reintroduced (added 2026-07-22 re-review, guards
  the discount-removal that motivated this session's re-review).

**Integration gate (BLOCKING — real Grid + AP + Turn Manager + Unit + Combat):**
- **GIVEN** the real stack, **WHEN** a player builds an Economy Outpost with exactly `build_cost` AP,
  **THEN** `apply_action` atomically spends AP + places it Under-Construction; an unaffordable build
  leaves AP/Grid unchanged.
- **GIVEN** an Economy Outpost's timer reaching 0 at the owner's real start-of-turn, **THEN** that same
  turn's `ap_income` reflects it (e.g. 2 completed → income X; a 3rd completes → income
...
- **GIVEN** a real Under-Construction structure destroyed before completion, **THEN** removed from the
  real Grid that step, no AP refunded.
...
destroyed), Scouts-only at `production_cap 2`, income = `BASE_INCOME` (10) with 0 outpost bonus; it is
A's start-of-turn. "Reference play line" = each side follows the deterministic greedy tempo AI (the
...
| Does **parallel/simultaneous construction** need a per-turn build-order cap? | systems-designer | A one-turn AP windfall could start many builds at once; the no-banking rule largely closes this. Confirm against the build-queue implementation. |
...
| Re-validate **boom-vs-rush at the VS-pinned 14×16 board** whenever a structure cost moves (8–24 is the engine range for future map variety). | game-designer / level design | The map-size bimodality AP Economy flagged; structure costs are tuned against provisional prototype economics. |
