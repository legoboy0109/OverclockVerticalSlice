...
| **D3** | **Population cap** | **MOD** | `population-cap.md` (new) | Base infantry cap, the cost curve to raise it, and which of the faction's units are exempt from it |
| **D4** | **Income-curve parameters** | **MOD** | AP & Credits Economy (#3) | Additive deltas to `BASE_INCOME` and the outpost tier bonuses, modifying `credit_income`. **Unchanged from v1, including the intercept/slope split and the combined-ceiling rule** |
| **D5** | **Structure roster + costs** | **OWN + MOD** | Base & Production (#7) | A faction may author its own structures (the Solar Federation's autonomous defences vs. the Alliance's manned ones) *and* delta the shared ones' `build_cost` / `build_time` / `production_cap` |
...
| **OQ-14** | ★★ **Is the Solar Federation a null faction as written?** *"Income slightly lower, but units proportionally cheaper"* — if income is −20% and costs are −20%, the arithmetic is identical to the baseline and it will play identically. Its real differentiator is the **higher infantry cap** (same spend, more bodies). Recommend leaning on the cap as the identity and treating the income/cost pair as flavour — or making the ratio deliberately **non**-proportional. **Design call** | user + economy-designer | `factions/solar-federation.md` |
