...
> **Status**: Designed — revised 2026-07-22 following full `/design-review` re-review (verdict NEEDS REVISION, 3 blockers fixed in-file; pending re-review). History: Designed 2026-07-22 → MAJOR REVISION NEEDED (11 blockers) → revised → NEEDS REVISION re-review (CR-10 tier gap, win-check FSM gap, CR-6a gesture constraint) → revised (this version).
> **Author**: user + agents (design-system skill, lean review mode)
> **Last Updated**: 2026-07-22
> **Implements Pillar**: Pillar 3 — Readable Board, Deep Decisions (flagship); serves Pillar 1 (see the AP cost before you pay it)
...
**CR-4 — Contextual action menu.** On selection, the menu lists only the verbs that are *legal and affordable right now* for that entity: **Move** (if `reachable` non-empty), **Attack** (if `legal_targets` non-empty and not yet `has_attacked` and `can_afford(attack_cost)`), **Produce** (producers only, if `production_cap` remaining and any `producible_type` affordable with a legal deploy tile), **Cancel Build** (under-construction structures only), and **Wait** (always — ends this entity's involvement without spending). A verb the entity *has* but cannot use right now (e.g. Attack when unaffordable) is shown **disabled with its reason**, not hidden — so the player learns why an option is unavailable rather than wondering where it went.
...
**CR-8 — Affordability gating (named handoff from AP Economy).** Every action's availability routes through AP Economy's `can_afford(player, amount)`. An action the player cannot afford is shown **disabled/greyed with the AP shortfall visible**, never offered as clickable. (AP Economy's GDD explicitly defers this "shown available vs. greyed" behavior to this GDD.)
...
This interface **owns**: selection state, input routing, the action menu, overlay/preview rendering, and calling the atomic commit. It **never** owns: cost math, damage math, legality rules, or AP deduction — those live in the dependency systems. The contract per dependency:
...
| **AP Economy** | `can_afford(player, amount)`, `current_ap`, `income` | *(none — AP is spent inside each system's commit)* | Affordability gating on every menu entry; the live cost each preview subtracts from the displayed pool |
...
**This system deliberately owns no cost, damage, or legality arithmetic.** Per CR-2, every substantive formula lives in a dependency (`move_path_cost` → Movement, `damage_formula` → Combat, `ap_income` → AP Economy, cancel refund → Base & Production). What this interface *does* own is **read-only combination of those systems' pure query outputs into display values and enabled/disabled verb states** — no balance math, but real aggregation. Three display-only derivations follow.

### D-1 · `projected_remaining_ap` — post-action AP readout

`projected_remaining_ap = current_ap(player) − previewed_cost`

| Variable | Type | Range | Description |
|---|---|---|---|
| `current_ap(player)` | int | 0 – income_this_turn (10–~32) | Live AP Economy query at **render time** |
| `previewed_cost` | int | 0 – current_ap | The **exact, unmodified** return of the owning system's cost query for the previewed action (`reachable()`'s `min_cost`, the queried `attack_cost`/`DEFENSIVE_ATTACK_COST`, a `produce_cost`/`build_cost`/`research_cost`, or a `CANCEL_REFUND_RATE` refund) — **passed through, never re-derived** |
...
| `can_afford(player, previewed_cost)` | bool | {T, F} | AP Economy's literal query — never reimplemented locally |
| `action_enabled` | bool | {T, F} | Verb shown selectable (T) or **disabled-with-reason** (F) |

**Output:** boolean. When false, the disablement **reason** is sourced from *which* conjunct failed (illegal vs. insufficient AP) — both are already in-hand from the two queries, no new derivation. **When *both* conjuncts fail, both reasons are surfaced** (not just the first): showing only one would hide information the player needs, producing a second "gotcha" when they fix the first (Player Fantasy: no surprises — AC-8). **Worked example:** Sniper attack, `attack_cost` query = 2, `current_ap = 1`, target in range → Combat `is_legal = true`, `can_afford = false` → disabled, reason "insufficient AP". Same sniper with the target *also* out of range → both fail → disabled, showing **both** "out of range" and "needs 2, have 1".
...
`attack_possible_after_move(unit, tile) = NOT has_attacked(unit) AND (legal_targets(unit, from_tile = tile) is non-empty) AND can_afford(player, reachable(unit)[tile].min_cost + attack_cost(unit))`
...
There is still **no combined move+attack projected-AP *number*** and **no aggregate multi-action spend** in this system — there is no action queue, and the interface never sums two actions into one displayed AP figure or simulates owning-system balance math (CR-2 / the Pass-Through Invariant forbid it). D-3's honest after-move signal does **not** violate this: it is a **boolean** (attack possible: yes/no) built from a Combat legality query plus one exact AP-subtraction affordability check — it displays no synthesized post-move AP number and re-derives no formula. The interface *asks* Combat "could this unit hit anything from tile T?" and *asks* AP Economy "is move+attack affordable?"; it computes neither answer itself.

> **What this GDD requires of its dependencies (a constraint, not an assertion about their internals):** no dependency's commit path may charge more than its own preview query returned (CR-2), and `legal_targets(unit, from_tile)` must be a pure, side-effect-free query (see Dependencies). The interface cannot enforce these from the outside — they are contract obligations on Movement / Combat / Base & Production / AP Economy, verified in *their* test suites and an ADR.

### Pass-Through Invariant (governs D-1, D-2, D-3)

> `projected_remaining_ap`, `action_enabled`, and `attack_possible_after_move` MUST be computed **exclusively** from the literal return values of owning-system queries for the specific previewed action, evaluated **at render time** — never cached across a frame in which underlying state could change (CR-10), and never re-derived by re-implementing an owning formula. **The interface holds zero copies of any owning-system balance constant** (`move_cost`, `SOFT_MOVE_PENALTY`, `attack_cost`, `COVER_DR`, etc.). Any interface code path that references a balance constant by name is a violation of this GDD's core principle. This is a structural constraint — enforceable by code review / lint and a candidate implementation ADR, not merely a stated rule.
...
- **If an action is unaffordable** (`can_afford` false): the verb is disabled/greyed with the **AP shortfall visible** (e.g. "needs 2, have 1"), never clickable (CR-8, D-2).
...
- **If the player hovers a *disabled* action to see why** (and cost previews are shown for disabled actions too): `projected_remaining_ap` may be negative, but it must be labeled **"insufficient AP"**, never framed as "remaining AP will be −1." A disabled action never presents a positive-framed remaining-AP readout.
...
- **If the player presses End Turn with unspent AP**: End Turn is **allowed without blocking**; a dismissible, non-blocking **"X AP will be lost"** reminder is shown (configurable off — see Tuning Knobs). Unspent AP is discarded per `ap_reset_policy` (no banking).
...
| **AP Economy** (#3) | `can_afford(player, amount)`, `current_ap(player)`, `income(player)` (queries) | Hard — the affordability gate every verb routes through (D-2); the named "greyed vs. available" handoff |
...
**Purity enforcement (owed, not assumed):** CR-2/CR-10 lean on every preview query (`reachable`, `legal_targets`, `preview_damage`, `can_afford`, and the two new hypothetical-tile queries) being genuinely **side-effect-free** — idempotent, mutating no game state (no AP, no hp, no signals any other system treats as gameplay-meaningful). This interface cannot enforce that from the outside; it is a contract obligation on the dependency systems, verified in *their* test suites and named in the ADR — not something this UI's own tests can cover.

**Bidirectional-consistency check (verified 2026-07-22, re-confirmed by `/consistency-check` full scan same day):** Movement, Combat, Base & Production, AP Economy, and Game State & Turn Manager each already list Command & Action Interface as a downstream Hard dependent in their own Dependencies sections — reciprocity is clean for the *existing* interfaces, and the two reciprocal contracts above are now landed, not outstanding.

## Tuning Knobs

> **All knobs here are UX-feel values.** Per the Pass-Through Invariant, the Command & Action Interface owns **no balance constant** — cost/damage/income knobs live in Movement, Combat, Base & Production, and AP Economy. Changing any knob below alters *feel and legibility*, never game balance.
...
| Cancel Build | **A commit-class event, not a refusal.** Inverse of the Build stamp (same voice, reversed) — reads as a *deliberate undoing that refunds AP*, weighty like the Build stamp it mirrors. Do **not** file it with the illegal-click/back-out "nothing happened" sounds — it is a successful, AP-refunding commit (see CR-6a, States table) and belongs in the commit family. |
...
|---|---|---|---|
| AC-1 | GIVEN an owned unit with ≥1 legal action, WHEN selected, THEN the menu opens and every enabled verb independently satisfies `is_legal AND can_afford`; no failing verb appears enabled | Logic | CR-1, CR-4 |
...
| AC-4 | GIVEN any previewed action showing exact value N, WHEN committed, THEN the resource (AP or HP) changes by exactly N — tested once per action family (move/attack/build) | Logic | CR-2 |
...
| AC-16 | GIVEN the Build command invoked in Action phase, THEN each structure type shows `build_cost`+`build_time`, is affordability-gated, and placement preview restricts to `legal_build_tiles` with the two exclusion reasons distinguishable by glyph/pattern (not color alone) | Logic + Visual-Feel | CR-5, CR-9 |
| AC-17 | GIVEN an under-construction structure, WHEN selected, THEN the menu offers only Cancel Build showing exact `floor(build_cost × CANCEL_REFUND_RATE)` refund, AND the structure reads as unmistakably unfinished/inert with a turns-remaining numeral shown; a Completed structure never offers Cancel Build | Logic + Visual-Feel | CR-9, Edge |
| AC-18 | GIVEN the player performs the **distinct Cancel-Build gesture** (CR-6a), THEN `current_ap` increases by exactly the previewed refund and the interface returns to ENTITY_SELECTED (refreshed) or IDLE; AND a **bare single left-click never triggers Cancel Build** | Logic | CR-6a, CR-9, States |
...
| AC-23 | GIVEN unspent AP > 0, WHEN End Turn pressed, THEN a non-blocking reminder shows, End Turn proceeds, unspent AP is discarded (not carried) | Logic + Visual-Feel | Edge, `UNSPENT_AP_REMINDER` |
...
> 1. D-3's predicate composes Combat's `legal_targets(unit, from_tile)` + AP Economy's `can_afford` — it never *re-implements* targeting or cost math locally.
